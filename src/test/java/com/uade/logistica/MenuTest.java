...
class MenuTest {
    ...
    void ejecutaOpcionesRestauradasYDepositos() {
        ...
        try (
                PrintStream captura = new PrintStream(salida);
        ...
        assertTrue(texto.contains("Depositos cargados: 4"));

        assertTrue(
                texto.contains(
                        "Hub Central Buenos Aires (50) - Auditoria reciente"));

        assertTrue(
                texto.contains(
                        "Deposito Cordoba1 (20) - Visitado: requiere auditoria"));
        ...
        assertTrue(
                texto.contains(
                        "Ingrese un nivel entero no negativo."));
    }
}